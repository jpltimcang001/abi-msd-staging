...
/**
    ...
    public function queueRetreieveStockInSalesmanExternal(Request $request)
    {
        ...
        $data = json_decode($request->getContent(), true);
		$params = [];
		$company = isset($data['company']) ? $data['company'] : ( isset($data['company_name']) ? $data['company_name'] : "BII Live");
        $file_name = DownloadConnector::PATH . date("YmdHis-") . str_replace(" ", "-", InventoryData::MODULE_INVENTORY . "-RETURN-REQUEST");
        Globals::saveJsonFile($file_name, $data);
		
		if($data == null) {
                return [
                    "success" => false,
                    "message" => InventoryData::MODULE_NAME_INVENTORY . " data sent is empty."
                ];
		}
    ...
    public function queueRetreieveInvoiceAndPickNoteExternal(Request $request)
    {
        ...
        try {
            /** Dispatches new Job */
            dispatch((new APIJobHandler('posted-pick-note-multiple', json_encode($data_inv)))->onQueue('api-queue-UAT'));
            return [
                "success" => true,
    ...
    {
        $data = json_decode($request->getContent(), true);
        $company = isset($data['company']) ? $data['company'] : "UAT";
     ...
     * @return - returns success message if able to complete without exceptions  
     */
    public function queueGenerateRunStockSplit(Request $request)
    {
        $data = json_decode($request->getContent(), true);
        $company = isset($data['company']) ? $data['company'] : "";
        ...
        try {
            /** Dispatches new Job */
            $test = dispatch((new APIJobHandler('stock-split', $request->getContent()))->onQueue('api-queue-' . strtolower($company)));
            return [
                "success" => true,
                "message" => "Stock Split successfully created queue to execute on background."
            ];
        } catch (\Exception $exc) {
            Log::error($exc->getMessage());
            throw $exc;
     ...
     * @return - returns success message if able to complete without exceptions  
     */
    public function queueCreditMemoPost(Request $request)
    {
        $data = json_decode($request->getContent(), true);
        $company = isset($data['company']) ? $data['company'] : "";
        ...
        try {
            /** Dispatches new Job */
            $test = dispatch((new APIJobHandler('credit-memo-post-queue', $request->getContent()))->onQueue('api-queue-' . strtolower($company)));
            return [
                "success" => true,
                "message" => SalesOrderData::MODULE_NAME_SALES_ORDER . " successfully created queue to execute on background."
            ];
