    ...
    public function handle()
    {
        switch ($this->argument('type')) {
            ...
            case "withdraw-stock-salesorder":
                ...
                break;
            case "credit-memo-post-queue":
                $this->runCreditMemoBatch();
                break;
            case "cash-reciept-post-queue":
                $this->runCashReceiptBatch();
                break;
            case "credit-memo-queue-new":
                $this->runCreditMemoQueue();
                break;
            default:
                return;
        }
...
al     * 
	 ...
	 * @return void 
	 */
	public function runCreditMemoPostQueue($data = null, $trigger = null)
	{
		...
		try {
			...
			if (!$from_batch) Utils::saveLog($trigger_id, $sales_office_no, date("Y-m-d H:i:s"), UploadConnector::INFO, UploadConnector::MSD_LOGGER_NAME, "Started sync " . strtolower(SalesOrderData::MODULE_NAME_SALES_ORDER) . " transaction.", ""); /* Save log message */
			UploadConnector::syncMSDSalesCreditMemoPostQueue( $data, $trigger_id); /* Call MSD REST API then process the response */
			if (!$from_batch) {
				Utils::saveLog($trigger_id, $sales_office_no, date("Y-m-d H:i:s"), UploadConnector::INFO, UploadConnector::MSD_LOGGER_NAME, "Done synced " . strtolower(SalesOrderData::MODULE_NAME_SALES_ORDER) . " transaction.", ""); /* Save log message */
				Utils::updateTriggerStatus($trigger_id, UploadConnector::STATUS_DONE); /* Update trigger status */
				Utils::updateTriggerEndDate($trigger_id, date("Y-m-d H:i:s")); /* Update trigger end date */
     ...
     * @return void 
     */
    public function runCreditMemoQueue($data = null, $trigger = null)
    {
        $data = json_decode($this->argument('data'), true);
        $method = "POST";
        $route = Params::values()['webservice']['abi_msd']['route']['sales-order']['list'];
        ...
        try {
            ...
            if (!$from_batch) Utils::saveLog($trigger_id, $sales_office_no, date("Y-m-d H:i:s"), UploadConnector::INFO, UploadConnector::MSD_LOGGER_NAME, "Started sync " . strtolower(SalesOrderData::MODULE_NAME_SALES_ORDER) . " transaction.", ""); /* Save log message */
            UploadConnector::syncMSDSalesCreditMemoQueue($data, $trigger_id); /* Call MSD REST API then process the response */
            if (!$from_batch) {
                Utils::saveLog($trigger_id, $sales_office_no, date("Y-m-d H:i:s"), UploadConnector::INFO, UploadConnector::MSD_LOGGER_NAME, "Done synced " . strtolower(SalesOrderData::MODULE_NAME_SALES_ORDER) . " transaction.", ""); /* Save log message */
                Utils::updateTriggerStatus($trigger_id, UploadConnector::STATUS_DONE); /* Update trigger status */
                Utils::updateTriggerEndDate($trigger_id, date("Y-m-d H:i:s")); /* Update trigger end date */
     ...
     * @return void 
     */
    public function runCashReceiptPostQueue($data = null, $trigger = null)
    {
        $data = json_decode(json_encode($data), true);
        $method = "POST";
        $route = Params::values()['webservice']['abi_msd']['route']['sales-order']['list'];
        ...
        try {
            ...
            if (!$from_batch) Utils::saveLog($trigger_id, $sales_office_no, date("Y-m-d H:i:s"), UploadConnector::INFO, UploadConnector::MSD_LOGGER_NAME, "Started sync " . strtolower(SalesOrderData::MODULE_NAME_SALES_ORDER) . " transaction.", ""); /* Save log message */
            UploadConnector::syncMSDCashReceiptJournalPostQueue($data, $trigger_id); /* Call MSD REST API then process the response */
            if (!$from_batch) {
                Utils::saveLog($trigger_id, $sales_office_no, date("Y-m-d H:i:s"), UploadConnector::INFO, UploadConnector::MSD_LOGGER_NAME, "Done synced " . strtolower(SalesOrderData::MODULE_NAME_SALES_ORDER) . " transaction.", ""); /* Save log message */
                Utils::updateTriggerStatus($trigger_id, UploadConnector::STATUS_DONE); /* Update trigger status */
                Utils::updateTriggerEndDate($trigger_id, date("Y-m-d H:i:s")); /* Update trigger end date */
     ...
     * @return void
     */
    public function runStockSplit($data = null, $trigger = null)
    {
        $data = ($data === null) ? json_decode($this->argument('data'), true) : $data;
        $method = "POST";
        $route = Params::values()['webservice']['abi_msd']['route']['reservation-entry']['list'];
        ...
        try {
            ...
            UploadConnector::syncMSDSplitting($method, $url, $data, $trigger_id); /* Call MSD REST API then process the response */
            if (!$from_batch) {
    ...
    public function runSalesOrderBatch()
    {
	 ...
	 /** 
		...
		public function runCreditMemoBatch()
		{
			...
			$trigger_id = isset($trigger['id']) ? $trigger['id'] : 0;
			Utils::saveLog($trigger_id, $sales_office_no, date("Y-m-d H:i:s"), DownloadConnector::INFO, DownloadConnector::MSD_LOGGER_NAME, "Started sending " . strtolower("Credit Memo Queue POST") . ".", ""); /* Save log message */
		
		
			print_r("[" . date("Y-m-d H:i:s") . "] Syncing Credit Memo Post \n");
			$this->runCreditMemoPostQueue($data, $trigger);
		}
	 ...
	 /** 
		 ...
		 * @return void
		 * */
		public function runCashReceiptBatch()
		{
			$data = json_decode($this->argument('data'), true);
			$sales_office_no = isset($data['sales_office_no']) ? $data['sales_office_no'] : "";
			$added_by = isset($data['added_by']) ? $data['added_by'] : "";
			...
			$trigger = Utils::saveTrigger($sales_office_no, "Credit Memo Queue", DownloadConnector::STATUS_PENDING, $added_by);
			$trigger_id = isset($trigger['id']) ? $trigger['id'] : 0;
			Utils::saveLog($trigger_id, $sales_office_no, date("Y-m-d H:i:s"), DownloadConnector::INFO, DownloadConnector::MSD_LOGGER_NAME, "Started sending " . strtolower("Cash Receipt") . ".", ""); /* Save log message */
		
		
			print_r("[" . date("Y-m-d H:i:s") . "] Syncing Cash Receipt \n");
			$this->runCashReceiptPostQueue($data, $trigger);
		}
    /** ------------------ Batch Sequence ~ End ---------------------- */
}
