...
class BalanceRequest extends Command
{
     ...
     * @var string
     */
    protected $signature = 'apirun:balanceRequest {--date-from=0} {--date-to=0} {--company=} {--sales-office=} {--salesman-code=} {--so-code=}';
     ...
     * @var string
     */
    protected $description = 'Run sync of Balance Requests';
    ...
    public function handle()
    {
        ...
        $salesman =  $this->option('salesman-code');
        $so_code =  $this->option('so-code');
        ...
        if ($company == "" || $sales_office_no == "") {	
            ...
            if (count($so_soap_result) > 0) {
                foreach ($so_soap_result as $v_so) {
                    ...
                    $data['so_code'] = $v_so->short_code;
					$data['salesman'] = $salesman;
                    /** Send queue */
                    Artisan::call(
                        'apirun:send',
                        [
                            'type' => 'batch-balance', 'data' => json_encode($data)
                        ]
                    );      
                }
            }
