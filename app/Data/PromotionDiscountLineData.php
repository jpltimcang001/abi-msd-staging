...
class PromotionDiscountLineData
{
    const MODULE_PROMOTION_DISCOUNT = "PROMOTION DISC.";
    const MODULE_NAME_PROMOTION_DISCOUNT = "Promotion Discount";
    ...
    {
        if (array_key_exists($key, PromotionDiscountLineData::dict)) {
            $key_val = PromotionDiscountLineData::dict[$key];
            return $this->$key_val;
        } else
            return null;
    }
    ...
    {
        return array_key_exists($key, PromotionDiscountLineData::dict);
    }
    ...
    {
        return array_search($key, PromotionDiscountLineData::dict);
    }
    ...
    {
        $xml_string = '<GetPromotionDiscountCriteria xsi:type="urn:GetPromotionDiscountCriteria">';
        foreach (PromotionDiscountLineData::dict as $key => $value) {
            $xml_string .= "<" . $value . ">" . ((isset($this->$value) || $this->$value !== NULL) ? htmlspecialchars($this->$value, ENT_XML1 | ENT_COMPAT, 'UTF-8') : "") . "</" . $value . ">\n";
        }
        foreach (PromotionDiscountLineData::dict_ext as $value2) {
            $xml_string .= "<" . $value2 . ">" . ((isset($this->$value2) || $this->$value2 !== NULL) ? htmlspecialchars($this->$value2, ENT_XML1 | ENT_COMPAT, 'UTF-8') : "") . "</" . $value2 . ">\n";
        }
        $xml_string .= '</GetPromotionDiscountCriteria>';
        return  $xml_string;
    }
}
