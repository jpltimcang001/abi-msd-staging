...
class ReturnRequest extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'apirun:ReturnRequest {--date-from=0} {--date-to=0} {--company=} {--sales-office=}';
     ...
     * @var string
     */
    protected $description = 'Run sync of Return Requests';
            ...
            if (count($so_soap_result) > 0) {
                foreach ($so_soap_result as $v_so) {
                    ...
                    Artisan::call(
                        'apirun:send',
                        [
                            'type' => 'return-request', 'data' => json_encode($data)
                        ]
                    );      
                }
            }
