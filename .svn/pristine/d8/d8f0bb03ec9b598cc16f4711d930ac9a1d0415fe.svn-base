...
class CAFCreditLimit extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'apirun:CAFCreditLimit {--date-from=0} {--date-to=0} {--company=} {--sales-office=}';
     ...
     * @var string
     */
    protected $description = 'Run sync CAFCreditLimit';
            ...
            if (count($so_soap_result) > 0) {
                foreach ($so_soap_result as $v_so) {
                    ...
                    Artisan::call(
                        'apirun:send',
                        [
                            'type' => 'caf-credit-limit', 'data' => json_encode($data)
                        ]
                    );
                }
            }
