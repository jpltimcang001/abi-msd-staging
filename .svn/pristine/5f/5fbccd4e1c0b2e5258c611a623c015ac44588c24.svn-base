...
class WithdrawStockSalesOrder extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'apirun:WithdrawStockSalesOrder {--date-from=0} {--date-to=0} {--company=} {--sales-office=}';
     ...
     * @var string
     */
    protected $description = 'Run sync of Withdarwal -> Stock in Salesman -> Upload Sales Order';
            ...
            if (count($so_soap_result) > 0) {
                foreach ($so_soap_result as $v_so) {
                    ...
                    Artisan::call(
                        'apirun:send',
                        [
                            'type' => 'withdraw-stock-salesorder', 'data' => json_encode($data)
                        ]
                    );
                }
            }
